use crate::{
    ...
    SelectTwoModel, SelectorTrait, StringIden,
};
use sea_query::{
    ...
    M: FromQueryResult + Sized + Send + Sync,
{
    ...
    pub fn cursor_by<C>(self, entity: E, order_columns: C) -> Cursor<SelectModel<M>>
    where
        C: IntoIdentity,
    {
        Cursor::new(
    ...
    N: FromQueryResult + Sized + Send + Sync,
{
        ...
        C: IdentityOf<E>,
    {
        let primary_keys: Vec<(DynIden, Identity)> = <F::PrimaryKey as Iterable>::iter()
            .map(|pk| {
                (
                    SeaRc::new(StringIden(f_entity.table_name().to_string())),
        ...
        let mut cursor = Cursor::new(
            self.query,
            SeaRc::new(StringIden(e_entity.table_name().to_string())),
...
//         assert_eq!(
//             Entity::find()
//                 .cursor_by(Column::Id, fruit::Entity)
//                 .before(10)
//                 .first(2)
//                 .all(&db)
//                 .await?,
...
//                 .cursor_by(cake::Column::Id, fruit::Entity, cake::Entity,)
//                 .before(10)
//                 .first(2)
//                 .all(&db)
//                 .await?,
...
//                 .cursor_by(cake::Column::Id, fruit::Entity)
//                 .before(10)
//                 .first(2)
//                 .all(&db)
//                 .await?,
